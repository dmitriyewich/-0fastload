...
- **только в одиночной игре (не SA-MP):** запоминание последнего выбранного слота сохранения и автозагрузка при следующем старте; слот хранится в userfiles в файле `!0fastload_lastslot.bin` (совместимо с **portablegta**, путь берётся через игровой getter `0x744FB0`);
- совместимость с **portablegta**: если загружен `portablegta.asi` / `PortableGTA.asi`, после патчей выполняется однократный вызов инициализации userfiles через `0x744FB0`.
...
В GitHub попадает только код плагина и файлы сборки:

| Путь | Назначение |
|------|------------|
| `source/main.cpp` | логика fastload-патчей, хук одиночной игры для автослота, `DllMain` |
...
- В **SA-MP** логика автозагрузки последнего слота **не** включается (только fastload-патчи и анти-залипания).
- Чтобы сбросить автослот в одиночной игре, удалите `!0fastload_lastslot.bin` из каталога userfiles или перезапишите файл, выбрав другой слот в меню загрузки.
